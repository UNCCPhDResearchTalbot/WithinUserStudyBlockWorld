...
public class CharFuncs : MonoBehaviour {
	...
	void OnControllerColliderHit(ControllerColliderHit hit) {
		...
		Vector3 howmuch = thisChar.transform.right*3f*Time.deltaTime;//Vector3.left*3*Time.deltaTime;
		howmuch.y = 0f;
		
		if (howmuch.x == 0f && howmuch.z == 0f) {
			howmuch.x += 3f*Time.deltaTime;
		...
		if (following) {
			...
			if (moveToObj != null && (moveToObj == thisChar || moveToObj.transform.parent == thisChar)) {
				...
				if (isVisible()) { // only start walking if can see target
					...
					int which = UnityEngine.Random.Range(0,2); // x position
					Vector3 howmuch = thisChar.transform.right*3f*Time.deltaTime;//Vector3.left*10*Time.deltaTime;
					howmuch.y = 0f;
